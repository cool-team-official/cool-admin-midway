...
export class ScoreWithdrawalService extends BaseService {
  ...
  async createWithdrawal(
    ...
    try {
      ...
      const scoreWithdrawalConfig = this.scoreWithdrawalConfigList.find(
        item => item.id === type
      );
      ...
      this.logger.debug(this.TAG, '用户积分', {
        currentScore,
        required: scoreWithdrawalConfig.score,
      });
      ...
      // 获取用户邀请码
      const inviteCodeEntity = await this.inviteCodeEntity.findOneBy({
        createUserId: userId,
      });
      ...
      this.logger.debug(this.TAG, '邀请记录', {
        count: inviteRecordCount,
        required: scoreWithdrawalConfig.member,
      });
      if (inviteRecordCount < scoreWithdrawalConfig.member) {
        throw new CoolCommException(
          `今天邀请${
            scoreWithdrawalConfig.member - inviteRecordCount
          }人即可提现`
        );
      }
      ...
      this.logger.info(
        this.TAG,
        `用户 ${userId} 申请提现 ${scoreWithdrawalConfig.amount} 元，消耗 ${scoreWithdrawalConfig.score} 积分`
      );
    } catch (error) {
